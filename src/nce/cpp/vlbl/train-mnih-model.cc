...
public:
  ...
  double log_prob_word_context(unsigned word, RowVectorXf contextWordAvg) {
    ...
    if (inputFile.is_open()) {
      ...
      while (getline(inputFile, line)) {
        ...
        for (tgtWrdIx=0; tgtWrdIx<words.size(); ++tgtWrdIx) {
          ...
          double contextScore = log_prob_word_context(tgtWord, contextVecAvg);
          ...
          for (unsigned v=0; v<filtVocabList.size(); ++v) {
            ...
            double scoreWordContext = log_prob_word_context(vWord, contextVecAvg);
  ...
  void train_nce(vector<unsigned>& words, unsigned nCores, double rate) {
    ...
    for (tgtWrdIx=0; tgtWrdIx<words.size(); ++tgtWrdIx) {
      ...
      vector<unsigned> contextWords = context(words, tgtWrdIx, windowSize);
      double h = contextWords.size();
      /* If there is no context to learn from, leave */
      if (h < 1) continue;
      /* Get the sum of context vectors */
      RowVectorXf contextVecAvg(vecLen);contextVecAvg.setZero(vecLen);
      ...
      double lpThetaTgt = log_prob_word_context(tgtWord, contextVecAvg);
      ...
      for (unsigned j=0; j<numNoiseWords; ++j) {
        double lpTheta = log_prob_word_context(noiseWords[j], contextVecAvg);
        ...
        pThetaToNoiseGradProd += pThetaToNoise*wordVectors[noiseWords[j]]/h;
      }
      /* Calculating context word update */
      RowVectorXf delContextVec = pNoiseToThetaTgt * wordVectors[tgtWord]/h;
  ...
  train_on_corpus(string corpus, unsigned iter, unsigned nCores, double lRate) {
    ...
    for (unsigned i=0; i<iter; ++i) {
      ...
      string line, normWord;
      vector<unsigned> words;
      unsigned numWords = 0, printIf=50000000;
      if (inputFile.is_open()) {
        time(&start);
        while (getline(inputFile, line)) {
          /* Extract normalized words from sentences */
          ...
          if (numWords > printIf) {
            print_vectors("temp-tgt-only-n10.txt", wordVectors, indexedVocab);
            printIf += 50000000;
          }
        }
        inputFile.close();
