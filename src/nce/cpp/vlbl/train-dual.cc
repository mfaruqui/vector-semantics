...
class WordVectorLearner {
  ...
  vector<RowVectorXf> adagradTgtVecMem, adagradConVecMem;
  RowVectorXf adagradTgtBiasMem, adagradConBiasMem;
  /* For sampling noise words */
  AliasSampler sampler;
    
public:
      
  vector<RowVectorXf> wordTgtVectors, wordConVectors;
  RowVectorXf wordConBiases;
  ...
  void init_vectors(unsigned vocabSize, unsigned vecLen) {
    wordTgtVectors = random_vector(vocabSize, vecLen); 
    wordConVectors = random_vector(vocabSize, vecLen); 
    wordConBiases = random_vector(vocabSize);
    adagradTgtVecMem = epsilon_vector(vocabSize, vecLen);
    adagradConVecMem = epsilon_vector(vocabSize, vecLen);
    adagradConBiasMem = epsilon_vector(vocabSize);
  }
  ...
  double diff_score_word_noise(unsigned word, vector<unsigned>& contextWords) {          
    ...
    for (unsigned i=0; i<contextWords.size(); ++i) {
      sumScore += wordTgtVectors[word].dot(wordConVectors[contextWords[i]]);
      sumScore += wordConBiases[contextWords[i]];
    }
  ...
  double log_lh(string corpus, unsigned nCores) {
    ...
    for (unsigned i=0; i<filtVocabList.size(); ++i) {
      ...
      for (j=0; j<filtVocabList.size(); ++j) {
        ...
        double pairScore = wordTgtVectors[word1].dot(wordConVectors[word2]);
        pairScore += wordConBiases[word2];
  ...
  void train_word_vec(vector<unsigned>& words, unsigned nCores, double rate) {
    ...
    for (tgtWrdIx=0; tgtWrdIx<words.size(); ++tgtWrdIx) {
      ...
      unsigned vecSize = wordConVectors[0].size();
      RowVectorXf noiseScoreGradProd(vecSize);
      noiseScoreGradProd.setZero(vecSize);
      ...
      for (unsigned j=0; j<numNoiseWords; ++j) {
        ...
        noiseScoreGradProd += noiseScore * wordTgtVectors[noiseWords[j]];
      }
      /* Grad wrt bias is one, grad wrt contextWord is the target word */
      double updateInBias = 1 - wordContextScore - noiseScoreSum;
      RowVectorXf updateInVec = (1-wordContextScore) * wordTgtVectors[tgtWord];
      ...
      for (unsigned k=0; k<contextWords.size(); ++k) {
        ...
        adagradConVecMem[contextWord] += updateVecSquare;
        adagradConBiasMem[contextWord] += updateBiasSquare;
        /* Now apply the updates */
        RowVectorXf temp = adagradConVecMem[contextWord].array().sqrt();
        wordConVectors[contextWord] += rate * updateInVec.cwiseQuotient(temp);
        updateInBias *= rate / sqrt(adagradConBiasMem[contextWord]);
        wordConBiases[contextWord] += updateInBias;
      }
      /* Update the tgt word also as this is dual representation */
      RowVectorXf conVecSum(vecSize); conVecSum.setZero(vecSize);
      for (unsigned k=0; k<contextWords.size(); ++k)
        conVecSum += wordConVectors[contextWords[k]];
      updateInVec = (1-wordContextScore-noiseScoreSum)*conVecSum;
      /* update adagrad */
      updateVecSquare = updateInVec.array().square();
      adagradTgtVecMem[tgtWord] += updateVecSquare;
      /* update the tgt word vector */
      RowVectorXf temp = adagradTgtVecMem[tgtWord].array().sqrt();
      wordTgtVectors[tgtWord] += rate * updateInVec.cwiseQuotient(temp);
    }
  }
...
int main(int argc, char **argv){
  ...
  print_vectors("news_tgt.txt", obj.wordTgtVectors, obj.indexedVocab);
  print_vectors("news_con.txt", obj.wordConVectors, obj.indexedVocab);
  return 1;
}