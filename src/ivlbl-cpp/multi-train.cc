  ...
  public:
    ...
    void train_word_vectors(vector<int>& words, float rate) {
        ...
        for (int i=0; i<words.size(); ++i) {
            ...
            noiseWords = get_noise_words(words, numNoiseWords, vocabSize);
...
int main(){
    string corpus = "../10k";
    int window = 5, freqCutoff = 2, noiseWords = 10, vectorLen = 80, numIter = 5;
    float rate = 0.05;
    
    WordVectorLearner obj (window, freqCutoff, noiseWords, vectorLen);
    obj.train_on_corpus(corpus, numIter, rate);
    print_vectors("news-try-multi.txt", obj.wordVectors, obj.indexedVocab);
    return 1;
}