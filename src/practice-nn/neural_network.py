...
class NeuralNetwork:
    ...
    #Follows the resilient back-prop give in Riedmiller & Braun paper                
    def train_rprop(self, exampleList, epoch):
        ...
        for timeStep in range(epoch):
            for exampleNum, example in enumerate(exampleList):
                ...
                for i in range(self.numNeuralLayers-2, -1, -1):
...
class NeuralLayer:
    ...
    def compute_and_set_node_values(self, inputValues):
        ...
        self.reset_node_output_values([sigmoid(val) for val in inputValues])
    ...
    def calculate_and_set_delta_backprop(self, nextNeuralLayer, outputList=None):
        ...
        if outputList != None and nextNeuralLayer == None:
            for i, node in enumerate(self.nodesDelta):
                self.nodesDelta[i] = sigmoid_derivative(self.nodesInput[i])*(outputList[i]-self.nodesOutput[i])
        else:
            ...
            for i, node in enumerate(self.nodesDelta[:-1]):
                ...
                self.nodesDelta[i] = sigmoid_derivative(self.nodesInput[i])*errorFromNextLayer
        ...
        for i, node in enumerate(self.nodesDelta):
            ...
            for j, edgeWeight in iterTransMat:
                ...
                if self.rpropDelEDelW[i][j] * newDelEDelW > 0:
                    self.rpropDelta[i][j] = min(ETA_POS*self.rpropDelta[i][j], DEL_MAX)
                    ...
                    self.rpropDelta[i][j] = max(ETA_NEG*self.rpropDelta[i][j], DEL_MIN)
                    self.rpropEdgesDelta[i][j] *= -1
